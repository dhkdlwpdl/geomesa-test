...
class GeoMesaRelation(options: util.Map[String, String], schm: StructType, sQLContext: SQLContext)
  ...
  override def buildScan(requiredColumns: Array[String], filters: Array[Filter]): RDD[Row] = {
    ...
    val requiredColumnsString = requiredColumns.mkString(",")
    val colOptions= Map("requiredColumn"-> requiredColumnsString)

    val df = sqlContext.sparkSession.read.format("geov2withoutsrp").options(options).options(colOptions).load()
    df.rdd
  }
}
